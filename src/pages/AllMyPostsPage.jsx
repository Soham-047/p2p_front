  ...
  return (
    <div className="bg-gray-50 min-h-screen ">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        {/* CHANGED: Grid now activates on medium screens (md) */}
        <div className="grid grid-cols-1 md:grid-cols-3 md:gap-8">
          
          {/* Main Content (Left Column) */}
          {/* CHANGED: Column span now activates on medium screens (md) */}
          <main className="md:col-span-2">
            <div className="mb-6">
              <h1 className="text-3xl font-bold text-purple-600">All Your Posts, All in One Place</h1>
               
            </div>
            
            {loading && <p className="text-center text-gray-500 py-10">Loading your posts...</p>}
              ...
              <>
                ...
                {postsResponse.results.length > 0 && (
                    <div className="flex justify-between items-center mt-8">
                        <Button onClick={() => setPage(page - 1)} disabled={!postsResponse.previous}>Previous</Button>
                        <span className="font-medium text-gray-600">Page {page}</span>
                        <Button onClick={() => setPage(page + 1)} disabled={!postsResponse.next}>Next</Button>
                    </div>
                )}
              </>
            )}
          ...
          {/* CHANGED: Sidebar is now hidden only on small screens, visible on medium and up */}
